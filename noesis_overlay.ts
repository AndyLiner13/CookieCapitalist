...
// #region 📋 README
// Unified Noesis UI controller for Cookie Clicker overlay (top layer).
// Handles header (cookie count/CPS), +# popup system, and footer navigation.
// Rain cookies and finger ring are separate overlays with lower render orders.
...
class Default extends Component<typeof Default> {
  ...
  start(): void {
    ...
    // Listen for page change events from HeaderNav
    this.connectLocalBroadcastEvent(
      LocalUIEvents.changePage,
      (data: { page: PageType }) => this.navigateToPage(data.page)
    );
    ...
    log.info("Overlay initialized on page: " + this.currentPage);
  }
  // #endregion
  ...
  private navigateToPage(page: PageType): void {
    ...
    // Broadcast page change to other overlays (Shop, Leaderboard)
